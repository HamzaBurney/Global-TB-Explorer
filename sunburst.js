const tooltip1 = d3.select("#tooltip11");
...
function createSunburst(data) {
  ...
  const height = width;  // Set height same as width to keep the chart square
  const radius = width / 8;  // Radius proportional to the width
  ...
  const shuffledColors = shuffleArray(d3.schemeCategory10);
  const color = d3.scaleOrdinal(shuffledColors);
  // Compute the layout.
  const hierarchy = d3
    .hierarchy(data)
    .sum((d) => d.value)
...
function updateChart1(year) {
  d3.json(`Sunburst_Treemap_Dataset\\${year}_data.json`).then(createSunburst);
}
